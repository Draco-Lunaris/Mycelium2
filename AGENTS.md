...
- `mycelium-auth` — local accounts, OIDC, TOTP, WebAuthn, RBAC. **Phase 4 complete.** `LoginService` (throttle + TOTP + session in one entry point; password change invalidates sessions), `UserStore` (Argon2id, sealed master keys), `SessionManager` (CSRF tokens, constant-time verify), `ApiKeyManager` (SHA-256 hashed), `JwtKeys` (Ed25519, typed Role), TOTP/WebAuthn (counter persistence, atomic challenge consume), OIDC (state+nonce verified). Initial admin is created only via the web `/setup` page (no bootstrap files).
...
- `mycelium-librarian` — in-process book ingest worker. **Phase 7 complete.** `LlmClient` (OpenAI-compatible, Ollama default, 300s timeout, admin-configurable via ConfigStore key `"llm"`), `parse_outline`/`slugify`/`build_catalog` (LLM-assisted with TOTAL heuristic fallback — no path where an LLM error fails a job), `ingest` (full text once to service-key `/library/<slug>.md` as a raw FileRepo payload — never in the registry/index — + catalog `/<slug>/book.md` hub + `/<slug>/ch-<n>-<slug>.md` chapters in the `global:library` namespace), `LibrarianWorker` (per-user slots, atomic `claim_ingest_job` pending→running so concurrent runners can't double-run, boot sweep fails stale running + requeues pending, lost-staged-text jobs fail with re-upload detail).
...
- Phase 7 (librarian + book ingest) complete: 197 tests green, judge PASS (all deliverables satisfied w/ file:line evidence; exit criteria demonstrated by ingest_integration.rs), review findings fixed (stray .bind() in fail_stale_running_jobs, atomic claim_ingest_job vs concurrent-runner clobber, upload runs only its own job not the whole queue, extract_passage fence-awareness mirroring parse_outline). Web adds: multipart CSRF path (boundary-aware, 33MiB cap), POST /api/v1/ingest (admin, 32MiB book cap), GET /api/v1/ingest + /{id} (admin), GET /api/v1/passages (book:// anchors, shelf visibility global-read-or-admin), admin upload form + jobs table, ingest metrics, boot sweep in main.rs.
- Phase 8 (global bookshelves + search) complete: 200 tests green, judge PASS (all deliverables + exit criteria + security properties verified), review findings fixed (reflected HTML injection via unescaped scope in the new-concept form; global skills list now type: Skill-filtered). Architecture: service scope split into `global:skills` / `global:library` namespaces (ConceptStore::for_service + EncryptedIndex::for_service take a namespace param); stack texts are raw FileRepo payloads (no registry/index — 32MiB bodies never poison search); ConceptStore::list_prefix (LIKE w/ escaped literal prefix) for chapter trees. Web adds: /books browse page (DB-driven, users see global-read shelves only), scope-aware concept viewer/writer (user/skills/library; skills admin-write-only, library read-only for everyone, reads visibility-gated), cross-scope search (user+skills+library, scope-tagged results, private-shelf books post-filtered out for non-admins), skills page (private type: Skill filter, admin edit links), admin global-skills section.
- Phase 9 (polish, docs, CI) complete: 202 tests green, judge PASS (all deliverables satisfied or justified-partial; notes fixed: SIGTERM graceful shutdown now actually handled + verified in-container, /health docs corrected to public-by-design, LICENSE files added), security-auditor pass 0 Critical/2 Important (both fixed: .dockerignore, backup Cache-Control: no-store) + suggestions fixed (CLI backup 0600, 6 internal-error detail leaks now logged server-side), code-reviewer 0 Critical/2 Important (both fixed: CLI backup default output moved outside the data dir + tar.gz exclusion in the archive walker, docs.yml nav .md→.html links + markdown link rendering). Adds: real CLI (bootstrap-admin/migrate/backup/verify), mycelium-store::backup (tar.gz, symlink-safe, archive-exclusion), admin portal real backup download (spawn_blocking, no-store), Dockerfile (cargo-chef, non-root uid 1000, healthcheck, STOPSIGNAL SIGTERM — graceful shutdown verified via docker stop), docker-compose.yml, .dockerignore, GitHub Actions (ci.yml fmt/clippy/test MSRV 1.88 + release build + binary smoke test; docker.yml GHCR publish w/ semver tags; docs.yml Pages site), docs/ (7 guides), LICENSE (MIT OR Apache-2.0). Docker image built + smoke-tested in-container (non-root, HTTPS 200, CLI verify+backup, SIGTERM graceful).
- Phase 10 (librarian agent — v1 parity) complete: 215 tests green, judge PASS (after fixes: OpenAI tool_calls echo protocol — AssistantToolCalls turn precedes tool results, verified by a request-capturing mock test; chat JS moved to external /assets/chat.js — the CSP blocks inline scripts), code-reviewer 0 Critical/2 Important (both fixed: search-term cap 32 in the agent mirroring the MCP guard, static_schema Box::leak now cached per-process via OnceLock) + suggestions fixed (case-insensitive reserved names, empty-type validation, dead read_passage stub removed, always-empty sources field removed). Architecture: ALL knowledge operations flow through the librarian agent (v1's core design) — mycelium-librarian::agent (LLM tool-call loop, MAX_STEPS=12, query/mutate/chat modes, v1's OKF system prompt w/ layout+types+mode sections, tool errors → model recovery); agent tools (search_knowledge w/ layout-on-miss + 32-term cap, read_concept, list_directory, lint_knowledge, write_concept, patch_concept w/ frontmatter merge + section replace, delete_concept; read-only gating); MCP memory_query/add/update/maintain rewired agent-first w/ deterministic fallbacks (no LLM → old behavior, never hard-fails); web /chat page + POST /api/v1/chat (session+CSRF, 503 when LLM unreachable); index.md auto-regenerated on every put/delete (raw FileRepo payload, never a concept; reserved filenames rejected on put).
- Next: post-Phase-10 — optional follow-ups: streaming chat (SSE), agent trace recording (v1's .traces/), query cache + hot memory, dream (offline maintenance), read_passage as a real agent tool (needs the service-key library scope plumbed into the agent context).